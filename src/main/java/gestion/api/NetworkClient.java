...
public class NetworkClient {
	...
	public static Retrofit getRetrofitClientWithToken(String token) {
		if (retrofitWithToken == null) {
			OkHttpClient okHttpClient = new OkHttpClient().newBuilder().connectTimeout(60, TimeUnit.SECONDS).readTimeout(60, TimeUnit.SECONDS).writeTimeout(60, TimeUnit.SECONDS).addInterceptor((Chain chain) -> {
			...
			retrofitWithToken = new Retrofit.Builder().baseUrl(BASE_URL).client(okHttpClient).addConverterFactory(GsonConverterFactory.create(gson)).build();
		}
